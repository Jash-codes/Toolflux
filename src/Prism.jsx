...
const Prism = ({
  ...
  useEffect(() => {
    ...
    const fragment = /* glsl */ `
      ...
      void main(){
        ...
        mat2 wob = mat2(1.0);
        if (uUseBaseWobble == 1) {
          float t = iTime * uTimeScale; float c0 = cos(t + 0.0); float c1 = cos(t + 33.0); float c2 = cos(t + 11.0); wob = mat2(c0, c1, c2, c0);
        }
        for (int i = 0; i < 100; i++) {
    ...
    const program = new Program(gl, {
      vertex, fragment,
      uniforms: {
        iResolution: { value: iResBuf }, iTime: { value: 0 }, uHeight: { value: H }, uBaseHalf: { value: BASE_HALF }, uUseBaseWobble: { value: 1 }, uRot: { value: new Float32Array([1,0,0,0,1,0,0,0,1]) }, uGlow: { value: GLOW }, uOffsetPx: { value: offsetPxBuf }, uNoise: { value: NOISE }, uSaturation: { value: SAT }, uScale: { value: SCALE }, uHueShift: { value: HUE }, uColorFreq: { value: CFREQ }, uBloom: { value: BLOOM }, uCenterShift: { value: H * 0.25 }, uInvBaseHalf: { value: 1 / BASE_HALF }, uInvHeight: { value: 1 / H }, uMinAxis: { value: Math.min(BASE_HALF, H) }, uPxScale: { value: 1 / ((gl.drawingBufferHeight || 1) * 0.1 * SCALE) }, uTimeScale: { value: TS }
      }
    });
    ...
    const setMat3FromEuler = (yawY, pitchX, rollZ, out) => {
        ...
        const r00 = cy * cz + sy * sx * sz; const r01 = -cy * sz + sy * sx * cz; const r02 = sy * cx;
        const r10 = cx * sz; const r11 = cx * cz; const r12 = -sx;
        const r20 = -sy * cz + cy * sx * sz; const r21 = sy * sz + cy * sx * cz; const r22 = cy * cx;
    ...
    const onMove = (e) => {
      ...
      pointer.x = (e.clientX - ww / 2) / (ww / 2); pointer.y = (e.clientY - wh / 2) / (wh / 2); pointer.inside = true;
    };
    ...
    const render = (t) => {
      ...
      if (animationType === 'hover') {
        ...
        yaw = tScaled * (0.2 + 0.3);
        pitch = Math.sin(tScaled * (0.3 + 0.4) + 1) * 0.6;
        roll = Math.sin(tScaled * (0.1 + 0.2) + 2) * 0.5;
        program.uniforms.uRot.value = setMat3FromEuler(yaw, pitch, roll, rotBuf);
      } else {
  ...
  }, [height, baseWidth, animationType, glow, offset, noise, transparent, scale, hueShift, colorFrequency, hoverStrength, inertia, bloom, timeScale]);
